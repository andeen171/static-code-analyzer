...
class AnalyzerTest(StageTest):
    def generate(self) -> List[TestCase]:
        return [TestCase(args=[f"test{os.sep}test_1.py"], check_function=self.test_1),
                ...
                TestCase(args=[cur_dir + f"{os.sep}test"], check_function=self.test_common)]
    ...
    def test_common(self, output, attach):
        file_1 = cur_dir.lower() + f"{os.sep}test{os.sep}test_3.py"
        file_2 = cur_dir.lower() + f"{os.sep}test{os.sep}test_4.py"
        file_3 = cur_dir.lower() + f"{os.sep}test{os.sep}test_5.py"
        ...
        # test_3 file
        if not output[-1].startswith(f"{file_1}: line 9: {error_code_default_argument_is_mutable}"):
            return CheckResult.wrong(MUTABLE_ARG)

        # test_4 file
        if not output[0].startswith(f"{file_2}: line 2: {error_code_arg_name}"):
            return CheckResult.wrong(ARG_NAME)
